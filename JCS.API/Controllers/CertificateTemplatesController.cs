...
public class CertificateTemplatesController : ControllerBase
{
    ...
    public CertificateTemplatesController(ICertificateTemplateService service) { _service = service; }

    [HttpGet]
    public async Task<IActionResult> GetAllTemplates(CancellationToken token) { return Ok(await _service.GetAllAsync(token)); }
    ...
    {
        if (!await _service.DeleteAsync(id, token)) return NotFound(new { message = $"Template with id {id} not found." });
        return NoContent();
    }
}
