...
interface OnyxExecutiveCertificateProps {
  ...
  return (
    <span
      className={`inline-block border-b border-slate-500 leading-[1.5] px-1.5 text-center align-baseline ${
        filled ? 'font-semibold text-black border-black' : ''
      }`}
...
/**
 * "Onyx Executive" design (coded):
 * charcoal hairline double frame with gold corner ticks, centered
 * executive layout, charcoal badge with gold trim, matching seal.
 */
export function OnyxExecutiveCertificate({
  certificate,
  template,
  previewMode = false,
}: OnyxExecutiveCertificateProps) {
  ...
  const primaryColor = template.primaryColor || '#181c22';
  const goldColor = template.accentColor || '#b08d2e';
  ...
  const cornerTick = 'absolute w-7 h-7 pointer-events-none';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-4 border pointer-events-none" style={{ borderColor: primaryColor }} />
      <div className="absolute inset-[22px] border border-slate-300 pointer-events-none opacity-70" />
      {/* Gold corner ticks */}
      <span className={`${cornerTick} top-4 left-4 border-t-2 border-l-2`} style={{ borderColor: goldColor }} />
      <span className={`${cornerTick} top-4 right-4 border-t-2 border-r-2`} style={{ borderColor: goldColor }} />
      <span className={`${cornerTick} bottom-4 left-4 border-b-2 border-l-2`} style={{ borderColor: goldColor }} />
      <span className={`${cornerTick} bottom-4 right-4 border-b-2 border-r-2`} style={{ borderColor: goldColor }} />
      ...
      {/* Seal (bottom left, inside frame) */}
      {template.showRosetteBadge !== false && (
        <div className="absolute bottom-10 left-11 w-[70px] h-[70px] pointer-events-none drop-shadow-lg z-20">
          <svg viewBox="0 0 100 100" className="w-full h-full">
            <defs>
              <linearGradient id="onyxSeal" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <div className="relative z-10 h-full flex flex-col justify-between px-16 pt-10 pb-9 text-center">
        ...
        <div className="flex flex-col items-center shrink-0">
          <div
            className="w-11 h-11 rounded-full bg-white overflow-hidden flex items-center justify-center shadow p-0.5"
          ...
          <h2 className="mt-1.5 text-[12px] font-bold tracking-[0.22em] uppercase text-slate-900 font-sans leading-tight">
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center space-y-2 overflow-hidden">
          <span
            className="inline-block px-8 py-[5px] text-[11px] font-bold tracking-[0.24em] uppercase whitespace-nowrap"
            style={{ backgroundColor: primaryColor, color: '#ffffff', border: `1px solid ${goldColor}` }}
          >
            {template.typeBadgeText}
          </span>

          <h1 className="text-[24px] font-black uppercase tracking-tight text-slate-900 font-sans leading-[1.2] max-w-2xl">
            {eventName}
          </h1>
          ...
          <p className="w-full text-center text-[10.5px] text-slate-500 leading-snug font-sans max-w-xl mx-auto line-clamp-2">
        ...
        <div className="flex items-end justify-between shrink-0">
          ...
          <div className="w-56 text-center">
            ...
            <div className="w-full mx-auto mb-1" style={{ height: 2, backgroundColor: goldColor }} />
            <div className="text-[12px] font-bold text-slate-900 leading-tight font-sans">
              {template.signature1Name}
            </div>
            <div className="text-[9.5px] font-medium text-slate-500 leading-tight">
