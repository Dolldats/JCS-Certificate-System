...
interface EmeraldFrameCertificateProps {
...
/**
 * "Emerald Ornate Frame" design — uses the user's framed artwork
 * (public/certificate-design2.png: cream canvas, green/gold ornate
 * border, gold seal) as a full-bleed background with live text on top.
 */
export function EmeraldFrameCertificate({
  certificate,
  template,
  previewMode = false,
}: EmeraldFrameCertificateProps) {
  ...
  const primaryColor = template.primaryColor || '#0c4a2e';
  const goldColor = template.accentColor || '#c9a227';
  const frameSrc = '/certificate-design2.png';
  ...
  return (
    ...
    >
      {/* Framed artwork background (ornate border + gold seal baked in) */}
      ...
      {/* Top padding clears the frame's center flourish ornament. */}
      <div className="relative z-10 h-full flex flex-col justify-between px-16 pt-[100px] pb-14 text-center">
        {/* Centered header */}
        <div className="flex items-center justify-center gap-3 shrink-0">
          <div
            className="w-10 h-10 rounded-full border-[3px] p-0.5 shrink-0 bg-white overflow-hidden flex items-center justify-center"
          ...
          <div className="text-left">
            <h2 className="text-[14px] font-black tracking-wide uppercase text-black font-sans leading-tight">
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center space-y-1.5 overflow-hidden">
          <h1
            className="text-[23px] font-black uppercase tracking-tight font-sans leading-[1.2]"
            style={{ color: primaryColor }}
          >
            {eventName}
          </h1>

          {/* Badge pill matching the emerald/gold frame */}
          <div className="inline-block pb-0.5">
            <span
              className="inline-block px-8 py-[5px] rounded text-[12px] font-bold tracking-[0.12em] text-white uppercase whitespace-nowrap shadow-sm"
              style={{
          ...
          <p className="w-full text-center text-[10.5px] text-slate-800 leading-snug font-sans max-w-2xl mx-auto line-clamp-3">
